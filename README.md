...
To use a local checkout (for example, if you've extended the server), point Codex at the repo's `cli.js`:

```toml
[mcp_servers.playwright-local]
command = "node"
args = ["/absolute/path/to/mcp-playwright/cli.js", "--headless"]
```

To keep desktop/mobile sessions separate, configure two servers:
...
#### Extra tool: responsive screenshot sweep

This repo additionally registers `browser_screenshot_sweep`, which captures a set of desktop/tablet/mobile screenshots (or custom viewports) and saves them into the server output directory.
...
#### Extra tool: save storage state (login reuse)

This repo also registers `browser_storage_state_save`, which saves the current browser context storage state (cookies + localStorage) into a JSON file in the output directory. This is useful for reusing authenticated sessions via `--isolated --storage-state=/path/to/storage.json`.
